...
namespace EmberCore.KernelServices.Plugins
{
    public class PluginsManager : IPluginsManager
    {
        ...
        private PluginResolver Resolver { get; }
        private EmberWpfUIService WpfUIService { get; }
        private readonly ILifetimeScope PluginLayerScope;
        private readonly LinkedList<Type> LoadedTypes = new();
        private readonly Dictionary<Type, string> LoadedTypeDescriptors = new();
        private readonly Dictionary<IPlugin, ILifetimeScope> PluginScopes = new();
        private readonly Dictionary<IPlugin, bool> PluginStatus = new();
        private readonly List<IEntryComponent> EntryComponents = new();
        private ILogger<PluginsManager> Logger { get; }
        private event Action<PluginDescriptor> PluginLoaded = null!;
        private event Action<PluginDescriptor> PluginUnloaded = null!;
        private event Action<PluginDescriptor> PluginInitialized = null!;
        public PluginsManager(ILifetimeScope scope, PluginResolver resolver, ILogger<PluginsManager> logger, EmberWpfUIService wpfUIService)
        {
            Resolver = resolver;
            Logger = logger;
